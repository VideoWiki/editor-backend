...
from sentence_detection.sentence_keyword_detect.scene_creation import scene_creation
...
def extract_info_from_ppt(file_path,file_name):
    ...
    for i in range(len(ppt_text)):
        ...
        if file not in files_in_dir:
            break
        if ppt_text[i] != '':
            scene_creation_data = scene_creation(ppt_text[i], 'long')
            keywords = scene_creation_data['keywords'][0]
            if not keywords:
                keywords = None
        else:
            keywords = None
        slides[i] = {"img_path": path +"-"+ str(i) + '.jpg', "keywords": keywords}
