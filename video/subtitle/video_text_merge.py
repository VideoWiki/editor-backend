...
def clip_chunks(txt, txt_position, script_color, bg_color, w, h, sceneDur,bg_opacity,transition_type):
    ...
    # center
    elif txt_position == 'center':
        clip = text_center(txt, script_color,
                           bg_color, w, h, sceneDur,bg_opacity,transition_type)

    # bottom
    elif txt_position == 'bottom':
...
def merger(script, url, font_color, background_color, text_position, bg_opacity, transition_type):
    file_name = str(uuid.uuid4())
    targetname = str(uuid.uuid4())
    txt = script
    script_color = font_color
    ...
    if video_url.startswith(BASE_URL):
        ...
        if video_url.endswith(".jpg") or video_url.endswith(".png") or video_url.endswith(".jpeg"):
            r = requests.get(video_url)
            # open(download_path + file_name + "." + video_url.split(".")[-1], "wb").write(r.content)
            with open(download_path + file_name + "." + video_url.split(".")[-1], "wb") as f:
                f.write(r.content)
    ...
    video_with_title_overlay.write_videofile(os.path.join(
        BASE_DIR, "media/edit-script/" + file_name + ".webm"),preset='ultrafast', threads=8, logger=None,
        ffmpeg_params=['-c:v', 'libvpx-vp9', '-crf', '30', '-b:v', '0', '-b:a', '120k', '-c:a', 'libopus', '-deadline', 'realtime', '-cpu-used', '8'])  # .mp4 to .webm
    generated_video = open(os.path.join(
        BASE_DIR, "media/edit-script/" + file_name + ".webm"), "rb")
    ...
    os.remove(os.path.join(BASE_DIR, "media/edit-script/" + file_name + ".webm"))
