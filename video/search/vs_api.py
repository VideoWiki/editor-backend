    ...
    def post(self,request):
        ...
        if request.data["type"]=="video":
            ...
            if r['totalHits'] > 0:
                for hit in r['hits']:
                    if hit['user_id'] not in userId:
                        ...
                        video_dict_sub[d] = {'id': id, 'thumbnail': img_url, 'url': video_url, 'tags': tags, 'current_tag': current_tag}
                        d += 1
                        e += 1
            ...
            if r['totalHits'] > 0:
                counter = 0
                for hit in r['hits']:
                    sub_dict[len(sub_dict)]={'id': hit['id'], 'url': hit["webformatURL"], 'tags': hit["tags"],'current_tag': current_tag}
                    counter += 1
                    if counter == 5:
                        break
